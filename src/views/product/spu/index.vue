...
watch(
  () => categoryStore.categoryIdThree,
  () => {
    if (!categoryStore.categoryIdThree) return
    getHasSpu()
  },
)
...
const addSpu = () => {
  scene.value = 1
  spu.value.initAddSpu(categoryStore.categoryIdThree)
}
...
const addSku = (row: SpuData) => {
  scene.value = 2
  sku.value.initSkuData(categoryStore.categoryIdOne, categoryStore.categoryIdTwo, row)
}
...
<template>
  ...
  <el-card style="margin: 10px 0">
    <div v-show="scene === 0">
      <el-button type="primary" size="default" icon="Plus" :disabled="categoryStore.categoryIdThree ? false : true"
        @click="addSpu">
        添加SPU
      </el-button>
      <el-table style="margin: 10px 0" border :data="records">
        ...
        <el-table-column label="SPU操作">
          <template #="{ row, $index }">
            <el-button icon="Plus" title="添加SPU" size="small" @click="addSku(row)"></el-button>
            <el-button type="primary" icon="Edit" title="修改SPU" size="small" @click="updateSpu(row)"></el-button>
            <el-button type="info" icon="View" title="查看SPU列表" size="small" @click="findSku(row)"></el-button>
